...
@foreach ($questions as $key => $question)
    <div class="quest_img">
        ...
        <span>Q. {{ $questions->currentPage()}}</span> <span class="sub"
            style="float: right;">{{ $question['subject'] }}</span><br>
        <hr>
        <img src="../../../images/questions/{{ $question['question_img'] }}" class="w-100" alt=""><br><br>
    </div>
    ...
    <div id="verifiBtn">
        @if ($questions->currentPage() == $questions->lastPage())
            <a class="btn col-auto btn-wave" href="#" onclick="myFunction()">Submit <i class="fa fa-arrow-right"
...
<script>
    function next_question() {
        ...
        console.log(question_awn);
        console.log(topic_awn);
        console.log(chcekk);
        $.ajax({
            url: "{{ asset('start-quiz-index/' . $topic->id . $questions->url($questions->currentPage() + 1)) }}" +
                "&question_id=" + question_awn + "&topic_id=" + topic_awn + "&user_answer=" + chcekk,
